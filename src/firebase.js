...
/** Counter for the controls */
const controlCounterRef = firestore.collection("counter").doc("control");
...
export function useStationControl(stage) {
  const station = stations[stage / 2];
  console.log(station);
  // Function Definition for Station Control
  const callForNextNum = async () => {
    // Authenticate the user
    if (!auth.currentUser) throw "You are not authenticated.";
    let transCount = 0;
    ...
    await firestore
      .runTransaction(transaction => {
        ...
        return transaction.get(controlCounterRef).then(async counter => {
          ...
          queryData.stage = queryData.stage + 1;
